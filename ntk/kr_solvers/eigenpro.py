...
import kr_solvers.svd as svd
import kr_solvers.utils as utils
...
class FKR_EigenPro(nn.Module):
    ...
    def fit(self, x_train, y_train, epochs, mem_gb,
        ...
        sample_ids = self.tensor(sample_ids).type(torch.long) # modified by jxyang
        ...
        for epoch in epochs:
            ...
            for _ in range(epoch - initial_epoch):
                ...
                for batch_ids in np.array_split(epoch_ids, n_samples / bs):
                    ...
                    batch_ids = self.tensor(batch_ids).type(torch.long) # modified by jxyang
            ...
            if run_epoch_eval:
                ...
                print("train error: %.5f\t (%d epochs, %.2f seconds)"%(tr_score['mse'], epoch, train_sec))
                
                res[epoch] = (tr_score['mse'])
